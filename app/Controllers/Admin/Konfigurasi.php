...
public function kpr()
{
    ...
    if ($this->request->getMethod() === 'post' && $this->validate([
        ...
        'kpr' => [
            'uploaded[kpr]',
            'mime_in[kpr,image/jpg,image/jpeg,image/gif,image/png]',
            'max_size[kpr,12096]',
        ],
    ])) {
        $file      = $this->request->getFile('kpr');
        $filename  = $file->getName();
        $filepath  = WRITEPATH . '../assets/upload/image/';
        $filethumb = $filepath . 'thumbs/';
        ...
        return redirect()->to(base_url('admin/konfigurasi/kpr'));
    }

    $data = [
        'title'       => 'Update Simulasi KPR',
        'konfigurasi' => $konfigurasi,
        'content'     => 'admin/konfigurasi/kpr',
    ];
    echo view('admin/layout/wrapper', $data);
}
...
public function struktur()
{
    ...
    if ($this->request->getMethod() === 'post' && $this->validate([
        ...
        'struktur' => [
            'uploaded[struktur]',
            'mime_in[struktur,image/jpg,image/jpeg,image/gif,image/png]',
            'max_size[struktur,12096]',
        ],
    ])) {
    ...
    $data = [
        ...
        'konfigurasi' => $konfigurasi,
        'content'     => 'admin/konfigurasi/struktur',
    ];
    echo view('admin/layout/wrapper', $data);
}
